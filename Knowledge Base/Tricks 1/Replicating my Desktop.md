...
Using `deje-dupe`, I made a backup to my [[NAS]] - just to be safe. I made another using [[Knowledge Base/Tricks 1/ZFS Backup]].  Just my user directory/zvol and a couple of known data directories, the OS will be handled with a fresh install. 
...
Lets get a VM setup that I can test on, for safety and prevent my bumbling from causing data-loss.  Using my trusty [[Knowledge Base/Tools/Proxmox]] server, this is an easy task.  I already had a copy of the 24.04.2 Ubuntu Desktop ISO on there, so I generated an instance to throw things at and then immediately made a snapshot so that I can roll back to the last stage when things go wrong. 
