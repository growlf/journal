...
### Grafana
I started with installing Grafana as a standalone [[Knowledge Base/LXC]] on my [[Proxmox 1]] server using a [script](https://community-scripts.github.io/ProxmoxVE/scripts?id=grafana). It looks like it is running fine at http://grafana.yeticraft.net:3000/. Nothing there though.  I need some data first.
### Prometheus
...
A good 20 minutes of playing around CAdvisor's interface later, I read the [storage instructions](https://github.com/google/cadvisor/blob/master/docs/storage/prometheus.md) to get all of that lovely data into [[Prometheus 1]] and thus into my friendly [[Grafana 1]] interface. Opening a terminal to the Prometheus container, I added the following to it's scrape config list:
```yaml
  - job_name: 'cadvisor'
